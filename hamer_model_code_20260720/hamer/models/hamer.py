...
class HAMER(pl.LightningModule):
    ...
    def training_step(self, joint_batch: Dict, batch_idx: int) -> Dict:
        ...
        if self.global_step % 100 == 0:
            ...
            for name, param in self.backbone.named_parameters():
                if param.grad is not None:
                    p_norm = param.grad.norm().item()
                    if not math.isnan(p_norm):
                        total_norm += p_norm ** 2
                    if 'mlgru' in name and not math.isnan(p_norm):
                        mlgru_norm += p_norm ** 2
                        mlgru_count += 1
            total_norm = math.sqrt(total_norm) if total_norm > 0 else 0
            mlgru_norm = math.sqrt(mlgru_norm) if mlgru_norm > 0 else 0
            print(f"Step {self.global_step}: Total grad norm={total_norm:.6f}, MLGRU grad norm={mlgru_norm:.6f}, MLGRU layers={mlgru_count}")
        # ========== END OF DIAGNOSTIC ==========
        ...
        if self.global_step % 50 == 0:
            losses = output['losses']
            log.info(f"Step {self.global_step} - "
                    f"Total: {losses['loss']:.4f}, "
                    f"3D KP: {losses['loss_keypoints_3d']:.4f}, "
                    f"2D KP: {losses['loss_keypoints_2d']:.4f}")
