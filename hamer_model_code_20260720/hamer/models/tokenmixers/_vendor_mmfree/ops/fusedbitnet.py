...
def activation_quant(x):
    ...
    This is PURE quantization without STE (matches original).
...
def _layer_norm_fwd_quant_kernel(
    ...
    # Apply quantization to the output (PURE quantization, NO STE in kernel)
...
class LayerNormLinearQuantFn(torch.autograd.Function):
    ...
    def forward(
        ...
        # === CHANGE 3.2: STE for WEIGHTS ===
        # Apply pure quantization first, then add STE
        linear_weight_pure = weight_quant(linear_weight)
        # STE: forward uses quantized weight, backward passes gradient through original weight
        linear_weight_ste = linear_weight + (linear_weight_pure - linear_weight).detach()
        linear_weight_ste = linear_weight_ste.to(dtype)
        # ==================================
        
        linear_bias = linear_bias.to(dtype) if linear_bias is not None else None
        
        # === STE for ACTIVATIONS (already present) ===
        y_quant = y + (activation_quant(y) - y).detach()
        
        out = F.linear(y_quant.to(linear_weight_ste.dtype), linear_weight_ste, linear_bias)
...
class BitLinear(nn.Linear):
    ...
    This matches the original MatMul-Free LM implementation.
    """
    ...
    def forward(self, x):
        """
        Overrides the forward pass to include quantization with STE.
        This matches the original MatMul-Free LM exactly.
        ...
        # Apply quantization with STE at call site (matches original)
...
class FusedBitLinear(BitLinear):
    """
    A custom linear layer that applies quantization on both activations and weights.
    This uses the fused kernel for better performance.
    """
    ...
    def forward(self, x):
        """
        Forward pass using fused kernel with STE applied at the call site.
        This matches the original MatMul-Free LM's pattern.
        """
        # Use the fused kernel which does norm + quant + linear
        # The STE is applied inside layer_norm_linear_quant_fn at the quantization step
