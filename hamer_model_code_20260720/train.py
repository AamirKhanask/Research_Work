...
from hamer.callbacks.weight_logger import BitLinearWeightLogger
import pyrootutils
...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    # Add weight logger only for MatMul-free experiments
    if cfg.MODEL.BACKBONE.get('TOKEN_MIXER_TYPE') == 'ss2d_mlgru':
        #from hamer.callbacks.weight_logger import BitLinearWeightLogger
        weight_logger = BitLinearWeightLogger(
            log_subset_size=100,      # Only log 100 random weights per layer
            log_every_n_steps=None,    # None = only log at start/end
            verbose=False                        
        )
